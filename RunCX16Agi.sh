...
LOOP_DEVICE="/dev/loop112"

# ---- Script ----

# Clean up any previous mount / loop device
if mountpoint -q "$SDCARD_MOUNT_POINT"; then
    echo "Unmounting previous mount..."
    sudo umount "$SDCARD_MOUNT_POINT" || sudo umount -l "$SDCARD_MOUNT_POINT"
fi
...
# Check/repair the filesystem (clears dirty bit that causes#!/bin/bash
set -euo pipefail
