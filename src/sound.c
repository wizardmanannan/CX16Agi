...
unsigned int b7GetLFSRFrequencyFromCX16Freq(unsigned int cx16Freq) {
	unsigned char b1Lsb;
...
unsigned int b7GetPeriodicFrequency(byte frequencyByte, unsigned int currentCh2Frequency)
{
	byte noiseFrequency = frequencyByte & 3;

	if (noiseFrequency != 3)
	{
		return b7GetLFSRFrequencyFromCX16Freq(b7NoiseFreq[noiseFrequency]);
	}

	return b7GetLFSRFrequencyFromCX16Freq(currentCh2Frequency);
}
	...
	while (b1FillNoteBuffer(oldChNoiseBuffer, oldChNoiseDataPtr, &oldChNoiseLocalBufferStatus, oldChNoiseBytes))
	{
		...
		else //periodic
		{
			for (i = 0; i < *((unsigned int*)&oldChNoiseBytes[DURATION_BYTE]); i++)
			{
				...
				noiseFrequency = b7GetPeriodicFrequency(oldChNoiseBytes[NOISE_CHANNEL], *((unsigned int*)&oldCh2Bytes[FREQUENCY_BYTE]));
