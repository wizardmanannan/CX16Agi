...
;This function is priority screen aware, each pixel will only should through if the pNum >= the 
;priority screen pixel at x,y.
...
@calculatePriorityAutoInc: ;This calculates whether the priority auto incrementment should be initially switched on or not. If X even auto inc should be off, because we need to read the same byte for the off next turn
lda X_VAL
lsr
bcs @oddValue
