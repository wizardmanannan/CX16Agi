...
For each game three pairs of files are produced (all uppercase):
    ...
    <GAMEID>-LOGIC-IDS.BIN / <GAMEID>-LOGIC-IDS.IDX
...
def strip_comments_and_strings(text: str) -> str:
    """Remove comments and string literals so keyword searches stay in code."""
    # // line comments
    text = re.sub(r'//[^\n]*', '', text)
    # [ ... ] AGI-style comments (common in original sources / some decompiles)
    text = re.sub(r'\[[^\]]*\]', '', text)
    # Double-quoted strings (handles escapes and spans newlines)
    text = re.sub(r'"(?:\\.|[^"\\])*"', '""', text)
